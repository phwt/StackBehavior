...
def readBatch():
    ...
    rootdir = './dataset/data/tag_counts/topic_based'
    for subdir, _, files in os.walk(rootdir):
        for file in files:
            ...
            with open(path, encoding="utf8") as csvfile:
                ...
                for row in readCSV:
                    write_file(row[0], row[1], path.split("_")[-1][:-4], row[2])

def write_file(year, quarter, name, count):
    """ Write data from csv reader """
    with open('./dataset/data/tag_counts/tags_count_topic_based_merged.csv', 'a', encoding="utf8", newline='') as csvfile:
